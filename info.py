...
API_ID = int(environ.get('API_ID', '6120228397'))
API_HASH = environ.get('API_HASH', '02739281cdc81c6562ed449dec7da88b)
BOT_TOKEN = environ.get('BOT_TOKEN', "8343449633:AAEabNSO-N99KiWEH2LYQY_COBShS2IicmU")
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '6120228397').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-6120228397] #Channel id for auto indexing ( make sure bot is admin )
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '').split()]
auth_channel = environ.get('AUTH_CHANNEL', '-6120228397) #Channel / Group Id for force sub ( make sure bot is admin )
...
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://<db_username>:darkmater@darkmater.k1ypktj.mongodb.net/?appName=Darkmater")
DATABASE_NAME = environ.get('DATABASE_NAME', "darkmater")
